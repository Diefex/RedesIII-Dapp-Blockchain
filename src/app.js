...
window.pagarDP = async () => {
    ...
    if(transferir('Universidad', mont)){}
}

window.transferir = async (dest, mont) => {
    const accounts = await web3.eth.getAccounts();
    ...
    document.getElementById('alertas').append(alerta);
    try{
        if(await Udicoin.methods.enviar(us, dest, mont).send({from: accounts[0]})){
